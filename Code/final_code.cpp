...
int main(int argc, char** argv)
{
	...
	else if((digitalRead(INPUT_PIN) == HIGH) && (state == LOW))
	{
	    ...
	    auto denoisedSignal = INRemoval(filteredSignal);//this is the impulse noise filter
    
	   //for(vector<int>::const_iterator i = denoisedSignal.begin(); i != denoisedSignal.end(); ++i)
	   //cout<<*i<< ' ';
	   
	   //the signal is now fully filtered....
	   
	    auto numberPeaks = peak_finder(denoisedSignal); //get the number of peaks in filtered signal
	    
	    int numPeaks = numberPeaks.size();
	    
	    int BPM = numPeaks *12.5;
		cout<<"BPM: "<<BPM<<endl;
	   ...
	   if (BPM<=30){
	    ...
	    temp.clear(); //clear out the vectors ready for the new samples
	    numberPeaks.clear(); 
