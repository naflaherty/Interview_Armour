...
function handler (req, res) { //create server
  //fs.readFile(__dirname + '/public/original_web.html', function(err, data) { //read file index.html in public folder
...
io.sockets.on('connection', function (socket) {// WebSocket Connection
  ...
  pushButton.watch(function (err, value) { //Watch for hardware interrupts on pushButton
    ...
    lightvalue = value;
    socket.emit('light2', lightvalue); //send button status to client
  });
...
io.sockets.on('connection', function (socket) {// WebSocket Connection
  ...
  pushButton.watch(function (err, value) { //Watch for hardware interrupts on pushButton
    ...
    lightvalue = value;
    socket.emit('light3', lightvalue); //send button status to client
  });
  socket.on('light3', function(data) { //get light switch status from client
    lightvalue = data;
    if (lightvalue != LED3.readSync()) { //only change LED if status has changed
